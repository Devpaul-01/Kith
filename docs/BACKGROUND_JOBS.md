...
> This document describes the queues, workers, and Redis usage patterns as currently implemented. Section 15 separately calls out planned future work.
...
All 9 queues, in the order declared in `queues/index.js`, along with their worker wiring (`background.workers.js`, `notification.worker.js`, `data_export.worker.js`) and service implementation.
...
![Recurring pool lifecycle: creation, rolling 3-month cycle generation with a nightly-maintenance fallback, the nightly open/close sweep with carry-forward, and the three admin override types with their differing timing](../media/kith-architecture-recurring-pool-lifecycle-diagram.png)
...
**A running system, not just a diagram.** The screenshot below shows the exact state this lifecycle produces in a real workspace: a closed cycle at 95% funded, an open cycle with a live per-participant paid/pending/overdue breakdown (including a balance carried forward from the prior cycle, shown inline against the member who owes it), and an upcoming cycle sitting at zero — the rolling 3-month horizon described above, observed rather than described. A matching admin override screenshot is in [PRODUCT_OVERVIEW.md §13](PRODUCT_OVERVIEW.md#13-participants--contribution-targets).
...
![Bull Board dashboard showing all 9 queues — notification, reminder, cycle-generation, cycle-lifecycle, task-overdue, invite-cleanup, engagement-check, notification-outbox, and data-export — each with live job counts](../media/kith-bull-board-nine-queues-live.png)
