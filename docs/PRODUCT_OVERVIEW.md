...
> **Document purpose:** This is not an architecture document, an API reference, or a developer guide. It is the definitive explanation of what Kith *does*, what problems it solves, and how people experience it — written so a hiring manager, product manager, designer, or investor can understand the entire product without reading a line of code.

> **Scope note:** This document describes the backend system as it exists today. Some capabilities implied by generic product-documentation templates — such as direct messaging/conversations, user blocking or reporting, and AI-driven features — are **not present** in this product and are intentionally omitted rather than speculated about.
...
![Members list showing 9 members with roles, an admin's engagement status, and two members explicitly tagged Proxy with a "managed by" admin](./media/members-list-proxy.png)
...
![Invite preview page reading "You're invited! Mom invited you to join The Adeyemi Family," listing the workspace's active containers with funding progress, and Sign in / Create account buttons](./media/invite-preview-page.png)
...
![Recurring pool container detail page showing monthly cadence, NGN budget target, 6 participants, and public page enabled](./media/recurring-pool-overview.png)
*A recurring pool's overview tab — "Monthly Family Support Fund," monthly cadence, a ₦200,000 budget target, and the public-page/show-names settings covered in [Public Sharing](#public-sharing) below.*
...
![Cycles list for a recurring pool showing a closed cycle, an open cycle with a per-participant paid/pending/overdue breakdown and a carried-forward balance, and an upcoming cycle](./media/cycles-list-breakdown.png)
...
A short walkthrough of this same pool running end-to-end — creation, automatic cycle generation, opening, closing, and carry-forward, with no manual step in between — is in [§25](#25-background-jobs--scheduling).
...
![Override Cycle 10 modal with Skip Member, Adjust Target, and Pause Pool options, a member selector, and an optional reason field](./media/cycle-override-modal.png)
...
![Possible Duplicate modal warning that a similar ₦30,000 contribution from Chidi was recorded 4 minutes earlier, with Cancel and Submit anyway options](./media/duplicate-detection-modal.png)
...
![Proof of Contribution modal showing a status stepper (Pending, Proof Uploaded, Confirmed), an uploaded receipt file marked Verified, and a Confirm contribution button](./media/proof-of-contribution-modal.png)
...
![Diagram of the ledger entry lifecycle: submission with idempotency and duplicate checks, admin confirmation, then a confirmed-immutable entry branching into dispute and correction paths](./media/architecture-ledger-lifecycle.png)
...
![Resolved dispute detail page showing the disputed carry-forward entry, the raiser's reason, a green resolution note from an admin, and a two-message notes thread](./media/dispute-detail-resolved.png)
*A resolved dispute end to end: Uncle Bayo disputes a ₦10,000 carried-forward balance he believes was already paid, Mom investigates via the notes thread, and the resolution note documents the outcome — a permanent record of both the disagreement and how it was settled, exactly as described above.*

**Full workflow demonstration.** The video below walks through this same lifecycle live in the application — a confirmed contribution being disputed, the urgent admin notification firing, the notes exchange, the admin's resolution, and (in a case requiring a numeric fix) the resulting correction entry and its appearance in the audit log:

<video src="./media/dispute-resolution-demo.mp4" controls width="720" poster="./media/dispute-detail-resolved.png"></video>

**Resolving a Financial Dispute in a Family Workspace** — demonstrates the complete dispute lifecycle: a confirmed contribution is disputed, all admins receive an urgent (in-app + push + email) notification, the admin reviews the entry and exchanges notes with the raiser, the dispute is resolved with a required note, and — where the resolution calls for it — a correction entry is posted against the original, which itself remains untouched. This single flow touches the ledger's immutability guarantee, the RBAC model, the notification system, and the audit trail all at once, which is why it's referenced again from [ARCHITECTURE.md §21.1](./ARCHITECTURE.md#211-recording-and-confirming-a-contribution-ledger-entry) rather than duplicated here.
...
![Kith dashboard showing overdue contributions, open disputes, member engagement, pending confirmations, active events, recurring pools, tasks, milestones, and recent activity](./media/dashboard-overview.png)
...
![Public share page for "Mom's 60th Birthday" showing the event date, ₦1,740,000 of ₦2,000,000 collected at 87%, and a per-contributor Paid/Pending list](./media/public-share-page.png)
...
A consolidated view of security-relevant behavior that's woven throughout the feature sections above, gathered here for completeness:
