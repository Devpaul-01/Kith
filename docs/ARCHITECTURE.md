...
**Scope:** Primarily the `src/` backend service. §24 covers the frontend at a stack level, based on `package.json`, config files, hooks, stores, services, guards, routing, and top-level app files — not a page-by-page breakdown of the frontend.
...
![Annotated system architecture diagram showing the React client, Express perimeter and auth chain, application layer, core domain, atomic financial operations, async job processing, and external services](./media/architecture-system-overview.png)
...
### 9.2 Core schema (inferred from query patterns)
...
![Diagram of notification fan-out per recipient and channel, showing the dedup check, in-app immediate delivery, push/email queuing, enqueue-failure handling, and the 5-minute outbox recovery scan feeding back into the worker path](./media/architecture-notification-outbox.png)
...
This section consolidates security-relevant decisions referenced individually elsewhere in this document, gathered here for anyone evaluating the system's security posture in one pass.
...
![Diagram showing ledger entry submission, admin confirmation, and the resulting confirmed-immutable entry branching into dispute and correction flows](./media/architecture-ledger-lifecycle.png)
*The same submission→confirmation flow as the sequence diagram above, redrawn to emphasize what happens *after* confirmation — the entry becomes immutable, and a dispute or a correction are the only two ways its effective state can subsequently change, each going through its own atomic Postgres RPC (§9.3). A recorded walkthrough of a real dispute — raise, urgent notification, admin review, resolution, correction — is linked from [PRODUCT_OVERVIEW.md §15](./PRODUCT_OVERVIEW.md#15-disputes--corrections).*
...
A diagram of this full lifecycle, plus the three admin override types layered on top of it, is in [BACKGROUND_JOBS.md §5.3–5.4](./BACKGROUND_JOBS.md#53-cycle-generation-queue), alongside a recorded demonstration of a recurring pool running unattended.
...
**Scope note:** based on `package.json`, Tailwind/PostCSS config, a set of hooks/stores/services/guards/router files, and the top-level app entry points. This section covers the frontend at the stack/architecture level rather than documenting every page-level component and all of `components/ui/*`.
...
*This document reflects the current state of the backend, plus a frontend stack overview (§24) based on `package.json`, config files, hooks, stores, services, guards, router, and top-level app files.*
