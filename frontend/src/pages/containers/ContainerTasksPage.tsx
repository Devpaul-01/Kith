...
export default function ContainerTasksPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: KEYS.tasks(workspaceId, id!),
    queryFn: () => taskService.list(workspaceId, id!),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: KEYS.tasks(workspaceId, id!) });
    ...
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="space-y-3">
        {tasks.map(t => (
          <div
            key={t.id}
            className="bg-white border border-border rounded-xl p-4 flex items-start justify-between gap-3"
          >
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {!isAdmin && t.status === 'pending' && (
                <button
                  ...
                  onClick={() => updateStatus.mutate({ taskId: t.id, status: 'in_progress' })}
                >
                  Start
                </button>
              )}
              {!isAdmin && t.status === 'in_progress' && (
                <button
                  ...
                  onClick={() => updateStatus.mutate({ taskId: t.id, status: 'completed' })}
                >
                  Complete
                </button>
              )}
      ...
      {/* ── Add Task Modal ────────────────────────────────────────────── */}
      <Modal open={showAdd} onClose={handleClose} title="New Task">
        <form onSubmit={handleSubmit(d => createMutation.mutate(d))} className="space-y-5">
          {/* Title — primary field, full attention */}
          <Input
            label="Title"
          ...
          {/* Description — secondary, kept last */}
          <Textarea
          ...
          {/* Assignment is done after creation via the task's reassign action */}
