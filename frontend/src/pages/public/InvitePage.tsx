...
export default function InvitePage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: KEYS.invitePreview(token!), queryFn: () => inviteService.preview(token!), staleTime: 600_000 });
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: () => showToast.error('Failed to accept invite.'),
  });
    ...
    <div className="bg-white rounded-2xl border border-border p-8 mt-8 text-center space-y-4">
      ...
      {session ? (
        ...
        <div className="space-y-2">
          ...
          <Button fullWidth onClick={() => nav(`/login`)}>Sign in to accept</Button>
          <Button variant="secondary" fullWidth onClick={() => nav('/signup')}>Create account</Button>
        </div>
      )}
    </div>
  );
