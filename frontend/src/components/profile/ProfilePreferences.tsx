...
export function ProfilePreferences({ user }: ProfilePreferencesProps) {
  ...
  // In ProfilePreferences.tsx - fix the toggle handlers
...
const updatePushEnabled = useMutation({
  ...
  onSuccess: (data: any) => {
    ...
    showToast.success(push_enabled ? 'Push notifications enabled' : 'Push notifications disabled');
  },
  onError: (error: any) => {
  ...
  onSettled: () => {
    // Optional: refetch user data to ensure consistency
  },
});
  ...
  return (
    <Card>
      ...
      <div className="space-y-4">
        ...
        {/* Make sure the onChange doesn't throw */}
