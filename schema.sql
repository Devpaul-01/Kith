...
-- TABLE: users
--
-- full_name is nullable because Google OAuth users may not have a name in their
-- metadata when POST /auth/register is first called. The application layer
-- enforces full_name for email/password users only (via signupSchema).
--
-- auth_provider is informational — Supabase is the authoritative auth source.
CREATE TABLE IF NOT EXISTS users (
  id                    UUID PRIMARY KEY,  -- matches Supabase auth.users.id
  email                 TEXT NOT NULL UNIQUE,
  full_name             TEXT,              -- nullable: OAuth users may fill this later
  bio                   TEXT,
  ...
  -- Which provider was used to create the account (informational only)
