...
async function reassignTask(req, res, next) {
  try {
    ...
    if (['in_progress', 'completed'].includes(prev.status)) {
      throw new BusinessRuleError(`Cannot reassign a task that is already '${prev.status}'`);
    }
...
//   1. Upfront batch participant validation (1 query instead of N)
//   2. Single batch insert for all valid tasks (1 query instead of N)
//   3. Notifications dispatched in parallel after the insert
// ─────────────────────────────────────────────────────────────────────────────
async function bulkCreateTasks(req, res, next) {
  try {
    ...
    // 1. Collect all unique assignee IDs and validate them in one query
    const assigneeIds = [...new Set(data.tasks.filter((t) => t.assigned_to).map((t) => t.assigned_to))];
    ...
    if (assigneeIds.length > 0) {
      const { data: participants } = await supabaseAdmin
        ...
        .eq('container_id', containerId)
        .in('workspace_member_id', assigneeIds);
      validParticipantIds = new Set((participants || []).map((p) => p.workspace_member_id));
    }

    // 2. Separate tasks that pass validation from those that don't
    const failed    = [];
    const toInsert  = [];

    data.tasks.forEach((t, index) => {
      if (t.assigned_to && !validParticipantIds.has(t.assigned_to)) {
        failed.push({ index, error: 'assigned_to is not a participant in this container' });
      } else {
        toInsert.push({ task: t, index });
      }
    });

    if (!toInsert.length) {
      return success(res, { created: [], failed }, 201);
    }

    // 3. Batch insert all valid tasks in a single query
    const { data: insertedTasks, error } = await supabaseAdmin
      .from('container_tasks')
      .insert(
        toInsert.map(({ task: t, index }) => ({
          container_id: containerId,
          title:        t.title,
    ...
    // 4. Fire all assignment notifications in parallel (non-blocking per task)
    const notificationPromises = (insertedTasks || [])
    ...
    success(res, { created: insertedTasks || [], failed }, 201);
  } catch (err) { next(err); }
}
