...
async function completeContainer(req, res, next) {
  try {
    ...
    // Auto-create milestone — failure is logged but does not abort the response
    ...
    if (milestoneErr) {
      logger.error('Failed to auto-create milestone on container completion', {
        containerId,
...
// ── Restore deleted container (admin only) ─────────────────────────
...
async function getSummary(req, res, next) {
  try {
    ...
    const shapedParticipants = (participants || []).map((p) => {
      ...
      // Non-admins see limited info for other members
