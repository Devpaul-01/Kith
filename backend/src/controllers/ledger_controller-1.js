...
async function createEntry(req, res, next) {
  try {
    ...
    // Requires the idempotency_key column — see migration note at top.
    // If the column does not yet exist the check is skipped gracefully.
    ...
    if (idempotencyKey) {
      try {
        ...
        if (existing) {
          // Return the previously created entry — safe to retry
    ...
    // Non-admins always contribute as themselves
    ...
    // Time-window duplicate detection (fallback when no idempotency key provided)
    ...
    // Notify admins on pending submissions
    if (status === 'pending') {
