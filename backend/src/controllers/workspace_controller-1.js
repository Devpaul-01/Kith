...
// ── Dashboard ─────────────────────────────────────────────────────
//
// Parallelized: all independent queries run concurrently via Promise.all.
// Pending confirmations is the only conditional query (admin only) and is
// included in the same Promise.all via a no-op for non-admins.
...
async function getDashboard(req, res, next) {
  try {
    ...
    // Shape pending confirmations (admin only)
    ...
    // Shape recent activity with readable descriptions
...
async function updateSettings(req, res, next) {
  try {
    ...
    // Batch upsert all settings in a single query
...
// ── Admin Announcement ─────────────────────────────────────────────
// Sends an admin_announcement notification to all active workspace members
// (optionally filtered by role). Useful for important broadcasts.
...
// ── Audit Log ──────────────────────────────────────────────────────
// Paginated, filterable audit log for admin review.
...
// ── Overdue Summary ────────────────────────────────────────────────
// Returns all members with outstanding overdue contributions across
// all active containers in this workspace, grouped by member.
...
async function getOverdueSummary(req, res, next) {
  try {
    ...
    // Fetch active participants with their current targets (no cycle targets)
    ...
    // Fetch all confirmed ledger entries for this workspace
    ...
    // Build paid map: `contributorId:containerId` → total confirmed
    ...
    for (const p of (participants || [])) {
      ...
      if (currentTarget.due_date >= today) continue; // not yet overdue
      ...
      if (!overdueByMember.has(p.workspace_member_id)) {
        overdueByMember.set(p.workspace_member_id, {
      ...
      entry.containers.push({
        ...
        container_name: p.containers?.name,
        outstanding,
    ...
    // Batch-fetch display names for overdue members
